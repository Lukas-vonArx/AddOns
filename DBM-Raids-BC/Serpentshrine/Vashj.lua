local mod	= DBM:NewMod("Vashj", "DBM-Raids-BC", 5)
...
mod:RegisterEventsInCombat(
	"SPELL_AURA_APPLIED 38280 38575 360327",--360327 will report invalid on retail, it's save to ignore.  (38511)
...
local function StriderSpawn(self)
	...
	timerStrider:Start(63, self.vb.striderCount)
	self:Schedule(63, StriderSpawn, self)
end

local function NagaSpawn(self)
	warnNaga:Schedule(42.5, self.vb.nagaCount)
	self.vb.nagaCount = self.vb.nagaCount + 1
	timerNaga:Start(47., self.vb.nagaCount)
...
function mod:SPELL_AURA_APPLIED(args)
	...
	elseif args:IsSpellID(38575, 360327) and args:IsPlayer() and self:AntiSpam() then
		specWarnToxic:Show(args.spellName)
		specWarnToxic:Play("watchfeet")
